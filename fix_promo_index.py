...
def fix_index(filepath):
    ...
    # Find the prices from the current index.html
    # Look for "Llevar 1 Unidad" block to extract the price
    m1 = re.search(r'<span class="block font-extrabold text-emerald-600 text-base">(.*?)</span>', content)
    if not m1: return
    offer_price = m1.group(1)

    m2 = re.search(r'<span class="block font-extrabold text-gray-900 text-base">(.*?)</span>', content)
    if not m2: return
    promo_2_price = m2.group(1)

    new_labels = f"""<div class="grid grid-cols-2 gap-2" id="promo-selector">
    ...
    old_modal_price = f'<span class="block text-base font-bold text-emerald-600">{offer_price}</span>'
    new_modal_price = f'<span id="modal-price" class="block text-base font-bold text-emerald-600">{offer_price}</span>'
    content = content.replace(old_modal_price, new_modal_price)

    script_insertion = f"""<script>
